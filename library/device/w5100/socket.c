/**
 * @file		w5100/socket.c
 * @brief		Socket Driver Source File - For w5100
 * @version	1.0
 * @date		2013/02/22
 * @par Revision
...
void device_mem_init(uint8 *tx_size, uint8 *rx_size)
{
	int16 i, mul;
	int16 ssum, rsum;
	uint8 rxSettings = 0, txSettings = 0;
	//	mul-1:				7 6 5 4 3 2 1 0
	//	2 bits per element: 3 0 0 0 2 0 1 0
	const uint16 sizeMulToSetting = 0xc084;
	...
	{
		DBGA("Sn_TXMEM_SIZE = %d, Sn_RXMEM_SIZE = %d", tx_size[i], rx_size[i]);

		SSIZE[i] = (int16)(0);
		RSIZE[i] = (int16)(0);

		if(ssum <= TOTAL_SOCK_MEM/2) {
			if(tx_size[i]==1 || tx_size[i]==2 || tx_size[i]==4 || tx_size[i]==8)
				mul = tx_size[i];
			else mul = 2;	// by Ssoo Default 2K --20120522
			SSIZE[i] = 0x400 * mul;
			SMASK[i] = 0x400 * mul - 1;
			txSettings |= (sizeMulToSetting >> ((mul-1)*2)) << (i*2);
		}
		if(rsum <= TOTAL_SOCK_MEM/2) {
			if(rx_size[i]==1 || rx_size[i]==2 || rx_size[i]==4 || rx_size[i]==8)
				mul = rx_size[i];
			else mul = 2;	// by Ssoo Default 2K --20120522
			RSIZE[i] = 0x400 * mul;
			RMASK[i] = 0x400 * mul - 1;
			rxSettings |= (sizeMulToSetting >> ((mul-1)*2)) << (i*2);
		}
	...
	IINCHIP_WRITE(WIZC_RMSR, rxSettings);
	IINCHIP_WRITE(WIZC_TMSR, txSettings);
}
